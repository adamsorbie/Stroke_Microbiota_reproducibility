...
plot_boxplot(df = alpha_div_meta, variable_col = "Group", value_col = "Faiths.PD", 
             ...
             col_palette = colours, group.order = c("Sham", "Stroke")) 
```
Statistical signficiance is calculated internally in the ```plot_boxplot```
...
an ordered vector of the group levels e.g. ```c("Sham", "Strok")```. Additionally,
we can provide the group colours to make interpretation easier. 

```{r}
plot_da(res_ancom, groups = c("Sham", "Stroke"), cols=colours)
```
