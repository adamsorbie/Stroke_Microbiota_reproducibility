...
```{r}
ancom_da2 <- function(ps, formula, group, ord=NULL, zero_thresh=0.3) {
...
```{r}
res_ancom <- ancom_da2(ps_norm, formula = "Group", group = "Group")
```
...
p <- plot_da(res_ancom, groups = c("Sham", "Stroke"), cols=colours) + 
  scale_y_continuous(breaks = seq(-3, 3, 1), limits = c(-4, 4))
p
ggsave("ancombc_da.pdf", p, device = "pdf",dpi=300, width = 8, height = 9)
```
